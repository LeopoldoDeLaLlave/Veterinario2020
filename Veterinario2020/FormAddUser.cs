...
namespace Veterinario2020
{
    public partial class FormAddUser : Form
    {
        ...
        {
            if (textBoxNombre.Text.Length>1 && textBoxNombre.Text.Length>1 && textBoxdni.Text.Length>5 && textBoxDir.Text.Length>5 && textBoxPass.Text.Length>3)
            {
                string myHash = BCrypt.Net.BCrypt.HashPassword(textBoxPass.Text, BCrypt.Net.BCrypt.GenerateSalt());

                
                c.addUser(textBoxNombre.Text, textBoxApellido.Text, textBoxdni.Text,textBoxEmail.Text,textBoxDir.Text, textBoxTelefono.Text, myHash);
